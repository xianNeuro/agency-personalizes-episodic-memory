...
> **Supplementary Results (expanded inline statistics):** [`manuscript_results_full_statistics.html`](manuscript_results_full_statistics.html) (and matching [`.pdf`](manuscript_results_full_statistics.pdf)) summarize Results claims with full *F* / *t* / *r*, degrees of freedom, *p* values, effect sizes (η², Cohen’s *d*), and 95% CIs in running text. Generated by [`run_all_analyses.py`](../../run_all_analyses.py) alongside [`comprehensive_analysis_report.html`](comprehensive_analysis_report.html) / [`.pdf`](comprehensive_analysis_report.pdf) and [`print_supplement.css`](print_supplement.css) for print layout.
...
- **95% Confidence Intervals (CI)**: Reported for all means and mean differences where applicable
  - Group means in ANOVA analyses (including RUN 1 recall and Romance engagement measures)
  - Mean differences in post-hoc t-tests (including RUN 2 ISC post-hocs with Cohen’s *d*)
  - One-sample t-test means
  - Pearson correlations: approximate 95% CI for *r* (Fisher *z*) in RUN 4, 9, and 10
